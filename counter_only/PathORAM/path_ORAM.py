import math
from counter_only.PathORAM.cruch_oram import CruchORAM
...
class PathORAM:
    ...
    def __init__(self, BLOCK_SIZE, number_of_blocks, allocate=False, is_map=False) -> None:
        self.number_of_blocks = 2**math.ceil(math.log(number_of_blocks,2))
        self.conf = config(BLOCK_SIZE, self.number_of_blocks, is_map)
        ...
        if (self.conf.N/self.conf.X)*self.conf.BALL_SIZE  < self.conf.LOCAL_MEMORY_SIZE:
            self.position_map = CruchORAM(BLOCK_SIZE, int(number_of_blocks/self.conf.X))
        else:
            self.position_map = PathORAM(BLOCK_SIZE, int(number_of_blocks/self.conf.X), allocate, True)
        if allocate:
